...
<body>
	<%------------------------------------------------
                        右边内容开始
    ------------------------------------------------%>
	<div class="demoTable layui-search-form" id="scrapWayPanel">
		<div class="layui-form-item">
			<div class="layui-inline">
				<label class="layui-form-label" style="width: 140px;">报废处理方式名称</label>
				<div class="layui-input-inline">
					<input type="text" name="scrapWayName" lay-verify="required"
...
<!-- 新增弹出框 -->
<script type="text/html" id="addScrapWayPanel">
      <form class="layui-form layerForm addScrapWayPanelForm myLabelWidth155" >
          <div id="carNoDivInsert" class="layui-form-item">
              <label class="layui-form-label">报废处理方式名称<span class="myRedColor">*</span></label>
              <div class="layui-input-inline">
                  <input type="text" name="scrapWayName" lay-verify="required" placeholder="此项必填" autocomplete="off" class="layui-input">
              </div>
          </div>
      </form>
  </script>
...
<script type="text/javascript">
      layui.use(['laydate', 'laypage', 'layer', 'table', 'carousel', 'upload', 'element','form','layedit'], function(){
          ...
          table.render({
              ...
              ,url:  basePath + '/scrapWay/pagingScrapWay' //数据接口
              ,cols: [[
                  {field: 'scrapWayName', title: '报废处理方式名称', sort: false}
                  ,{field: 'createRealName', title: '创建人', width:150, sort: false}
                  ,{field:'createTime', title: '创建时间', width: 250,height: 315
                      ,templet: function(d){
                  ...
                  ,{field: 'createOrgName', title: '创建公司', width:250, sort: false}
              ]]
              ,id: 'layuiReloadId'
              ,page: true
              //,height: 315
          ...
          var reloadTableData = function(currValue){
          	...
          	var reloadParam = {
                     where: {
                    	 'scrapWayName':$(".layui-search-form input[name='scrapWayName']").val()
                     }
              };
            	if(currValue){
          ...
          layui.$('.demoTable .resetButton').on('click', function(){
          	  $("#scrapWayPanel input[name='scrapWayName']").val("");
          });
          var layerEditName;
          //新增按钮点击
          layui.$('.demoTable  .addButton').on('click', function(){
              layerEditName = layer.open({
                  ...
                  ,content: $("#addScrapWayPanel").html() //'<div style="padding:50px;">这是一个非常普通的页面层，传入了自定义的html</div>'
                  ,cancel: function(){
                      //alert("关闭啦");
                  }
                  ...
                  ,btn1: function(index){
                	  var param = {};
                	  param["scrapWayName"] = $(".addScrapWayPanelForm input[name='scrapWayName']").val();
                      layer.load(2); //加载中loading效果
                      $.ajax({
                          url: basePath + "/scrapWay/addScrapWay",
                          type:"POST",
                          dataType: "json",
                          data: param,
